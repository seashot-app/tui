...
class Filter extends Submenu {
  ...
  _removeEvent() {
    ...
    snippet.forEachOwnProperties(this._els.ranges, (value, name) => {
      this._els.ranges[name].off();
    });

    snippet.forEachOwnProperties(this._els.colors, (value, name) => {
  ...
  addEvent({ applyFilter }) {
    ...
    this._els.other.removeColorThresholdRange.on(
      'change',
      changeFilterStateForRange('removeColor')
    );
  ...
  setFilterState(changedFilterInfos) {
    ...
    if (!isRemove) {
      this._setFilterState(type, options);
    }

    this.checkedMap[type].checked = !isRemove;
  }
  ...
  _changeFilterState(applyFilter, filterName, isLast = true) {
    ...
    if (this._getFilterType(filterName) === 'range') {
      this._els.ranges[filterName].disabled = !apply;
    }
  ...
  _makeControlElement() {
    ...
    filterMenu.range.forEach((name) => {
      ...
      FILTER_RANGE[name].disabled = true;
    ...
    filterMenu.color.forEach((name) => {
      const tag = toKebabCase(name);
      const colorpicker = new Colorpicker(this.selector(`.tie-${tag}-color`), {
        defaultColor: FILTER_COLOR[name],
    ...
    this._els.other.blendType = this._pickerWithSelectbox(
